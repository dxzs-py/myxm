...
class CountyModelAdmin(admin.ModelAdmin):
    list_display = ["name", "city_belong"]  # 显示所属城市
    list_filter = ["city_belong__province"]  # 按省份过滤县区 添加过滤器。
    search_fields = ["name", "city_belong__name"]  # 支持跨表搜索
    list_per_page = 10
    ordering = ["-id"]
...
class CityModelAdmin(admin.ModelAdmin):
    list_display = ["name", "province"]  # 显示所属省份 # 指定显示字段。
    # list_editable = ["is_show"]  # 设置可编辑字段。
    list_filter = ["province"]  # 按省份过滤城市  添加过滤器。
    search_fields = ["name", "province__name"]  # 支持跨表搜索 支持按字段搜索。
